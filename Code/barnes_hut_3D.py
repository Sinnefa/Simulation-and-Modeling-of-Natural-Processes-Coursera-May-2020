...
# or an abstract node of the quad-tree if it has child.
    ...
    def __init__(self, m, x, y, z): #Alberto
    # The initializer creates a child-less node (an actual body).
        ...
        self.m_pos = m * array([x, y, z]) # Alberto
        self.momentum = array([0., 0., 0.])
        self.child = None

    def __str__(self): # Alberto
...
#    plt.gcf().savefig('bodies_{0:06}.png'.format(i))
    ...
    ax.scatter([b.pos()[0] for b in bodies], [b.pos()[1] for b in bodies], [b.pos()[2] for b in bodies], marker='.', c="#2222ff44")
    ax.scatter(bodies[0].pos()[0], bodies[0].pos()[1], bodies[0].pos()[2], c='r', marker='*')
    ax.set_xlim([0.4, 0.6])
    ax.set_ylim([0.4, 0.6])
    ax.set_zlim([0.4, 0.6])
...
posz = random.random(numbodies) *2.*ini_radius + 0.5-ini_radius # Alberto
# We only keep the bodies inside a circle of radius ini_radius.
bodies = [ Node(mass, px, py, pz) for (px,py,pz) in zip(posx, posy, posz) if (px-0.5)**2 + (py-0.5)**2 < ini_radius**2 ] # Alberto
