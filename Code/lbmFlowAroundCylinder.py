...
###### Flow definition #########################################################
maxIter = int(sys.argv[2]) # 200000  # Total number of time iterations.
Re = int(sys.argv[1])         # Reynolds number.
...
for time in range(maxIter):
    ...
    #if (time > maxIter-2): # and 
    if (time%25==0):
        plt.clf()
        plt.imshow(sqrt(u[0]**2+u[1]**2).transpose(), cmap=cm.Reds)
        plt.savefig(sys.argv[1]+("vel.{0:04d}.png".format(time//25)) )
